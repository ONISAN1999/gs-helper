...
() => {
  ...
  document.querySelectorAll(".CompsBoardContainer").forEach(board => {
    ...
    // --- แผนอัปเลเวล ---
    const lvAll = card.innerText.split(/การอัปเลเวล|Leveling/)[1] || card.innerText;
    const plan = (lvAll.split(/ลำดับความสำคัญ|Item Priority|Priority/)[0] || "").trim();
    const steps = [...plan.matchAll(/(?:เลเวล|Level)\s*(\d+)[\s\n]+(\d-\d|-|—)/g)]
      .map(x => [+x[1], x[2] === "—" ? "-" : x[2]]);
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        page.goto(URL_COMPS, wait_until="domcontentloaded", timeout=90000)
        page.wait_for_timeout(6000)
        comps = scroll_and_collect(page, JS_PULL_COMPS)
        data["comps"] = comps
        print(f"คอมพ์ทั้งหมด {len(comps)}", flush=True)

        # เขียนไฟล์ชั่วคราวทันทีที่ได้คอมพ์ กันส่วนที่เหลือล้มแล้วไม่ได้อะไรเลย
        try:
            OUT.parent.mkdir(parents=True, exist_ok=True)
            OUT.write_text(json.dumps(data, ensure_ascii=False, indent=1), encoding="utf-8")
            print("เขียนคอมพ์ลงไฟล์ไว้ก่อนแล้ว", flush=True)
        except Exception as e:
            print("  เขียนไฟล์ชั่วคราวไม่สำเร็จ:", e, file=sys.stderr)

        print("เปิดหน้าอาร์ติแฟกต์…", flush=True)
        page.goto(URL_ART, wait_until="domcontentloaded", timeout=90000)
        page.wait_for_timeout(5000)
        last = 0
        for i in range(10):
            page.mouse.wheel(0, 1600)
            page.wait_for_timeout(1500)
            n = page.evaluate("() => document.querySelectorAll('a[href*=DA_Artifact_]').length")
        ...
        page.goto(URL_UNITS, wait_until="domcontentloaded", timeout=90000)
        page.wait_for_timeout(6000)
        ...
        page.goto(URL_AUG, wait_until="domcontentloaded", timeout=90000)
        page.wait_for_timeout(6000)
